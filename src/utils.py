...
def load_data(edgelist, node_features, node_labels):
    features = sp.csr_matrix(node_features, dtype=np.float32)  # drug and target features
...
def prepare_physics_diffusion(adj, diffusion_coef=0.1):
    """准备物理扩散所需的矩阵"""
    adj_normalized = physics_normalize_adj(adj + sp.eye(adj.shape[0]))
    L = sp.eye(adj.shape[0]) - adj_normalized  # 随机游走拉普拉斯

    # 扩散算子
    I = sp.eye(adj.shape[0])
    diffusion_operator = I + diffusion_coef * L
    return diffusion_operator, L


def physics_based_propagate(features, k, diffusion_operator):
    """基于物理扩散的特征传播"""
    feature_list = []
    ...
    for i in range(1, k):
        ...
        if torch.is_tensor(features):
            feat_np = feature_list[-1].cpu().numpy()
        else:
            feat_np = feature_list[-1]

        feat_next = []
        for dim in range(feat_np.shape[1]):
            try:
                u_dim = splinalg.spsolve(diffusion_operator, feat_np[:, dim])
                feat_next.append(u_dim)
            except:
                feat_next.append(feat_np[:, dim])

        feature_list.append(torch.FloatTensor(np.column_stack(feat_next)))
